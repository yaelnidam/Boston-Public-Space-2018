...
/*
      ...
      var xAxis = d3.axisBottom()
                    .scale(x)
                    .tickFormat(function(d) {return data[d].route;});
...
*/
    ...
    function draw_graphs(day_data) {
      ...
      for ( var ix=0; ix<3; ix++) {
        ...
        var tmp = [];
         ...
         for ( var i=0; i<day_data['temperature'].length; i++) {
         tmp.push(
      ...
      for ( var ix=0; ix<3; ix++) {
        if (new_data_nest[ix].key=='temperature'){
          for ( var i=0; i<new_data_nest[0].values.length; i++) {
              new_data_nest[ix].values[i]['si'] = '°F'
        ...
        else if (new_data_nest[ix].key=='elevation'){
          for ( var i=0; i<new_data_nest[0].values.length; i++) {
              new_data_nest[ix].values[i]['si'] = 'ft'
          }
        }
      ...
      function draw_base_map() {
        ...
        var center = [	42.3431617	,	-71.0931516	]  //[53.01,25.73]->Eastern Europe,  [37.77, -122.45]->California
   ...
   accessToken: accessToken
        ...
        // Mapbox with leaflet
        var map = L.map('map').addLayer(mapboxTiles).setView(center, 15);
